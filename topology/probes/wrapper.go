...
func NewProbeWrapperWithDuration(handler handler, interval int) *ProbeWrapper {
	return &ProbeWrapper{sm: newServiceManager(handler, time.Duration(interval)*time.Second)}
}
