...
const (
	// SwitchUP describes switches having reachable management address
	SwitchUP string = "UP"
	// SwitchInaccessible describes switches having unreachable management address
	SwitchInaccessible string = "INACCESSIBLE"
	// SwitchDOWN describes switches that are no longer be discovered by its neighbors, and having inaccessible management address
...
type Probe struct {
	...
	linkWatcher   *Watcher             // remember the last time an LLDP packet arrived on each link
}

// Watcher keeps track on frequently updated elements
type Watcher struct {
	sync.Mutex
	elements map[graph.Identifier]time.Time // mapping between an element's ID and the last moment it was updated
	interval time.Duration                  // watcher interval
	timeout  time.Duration                  // drop element after timeout
...
// UpdateElement store the last time an element gets updated
...
// DropElement lose track of an element
